...
function Stepper({ step }) {
  const steps = ["导入", "体检", "清洗", "特征", "推理", "解释", "交付"];
  return (
    <div className="w-full">
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={s} className="flex-1">
              <div className={cn("flex items-center gap-2", i === steps.length - 1 ? "justify-end" : "justify-start")}>
                <div
                  className={cn(
                    "h-8 w-8 rounded-2xl border flex items-center justify-center text-xs font-semibold",
                    active ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"
                  )}
                >
                  {i + 1}
                </div>
                <div className={cn("text-sm", active ? "text-foreground" : "text-muted-foreground")}>{s}</div>
              </div>
              {i < steps.length - 1 && (
                <div className="mt-2 pr-4">
                  <Progress value={active ? 100 : 0} className="h-1" />
                </div>
              )}
            </div>
          );
    ...
    <div className="sticky top-0 z-20 bg-background/80 backdrop-blur border-b">
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm text-muted-foreground">结构化数据智能工作流演示（Mock）</div>
...
function SideNav({ active, setActive }) {
  return (
    <div className="w-full md:w-64 shrink-0">
      <div className="md:sticky md:top-[76px] p-3">
        <Card className="rounded-2xl">
          <CardHeader>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          className="rounded-2xl"
          onClick={() => {
            setDatasetId(datasetId === "incident_wide_v1" ? "incident_wide_v2" : "incident_wide_v1");
            notify("数据版本已切换", datasetId === "incident_wide_v1" ? "已切换到 v2（清洗后）" : "已切换到 v1（含脏数据）");
          }}
        >
          一键切换 v1 / v2
        </Button>
        ...
        <Card className="rounded-2xl">
          <CardContent className="p-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-sm text-muted-foreground">平台形态：从数据到决策的流水线（可视化证据链）</div>
                <div className="mt-1 text-base font-semibold flex items-center gap-2">
                  <Sparkles className="h-5 w-5" />
                  <span>导入 → 体检 → 清洗 → 特征 → 推理 → 解释 → 交付</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge className="rounded-xl" variant="secondary">当前页面：{NAV.find((n) => n.key === active)?.name}</Badge>
                <Badge className="rounded-xl" variant="outline">场景：{SCENES.find((s) => s.id === sceneId)?.tag}</Badge>
                <Badge className="rounded-xl" variant="outline">数据：{datasetId === "incident_wide_v2" ? "v2" : "v1"}</Badge>
              </div>
            </div>
            <div className="mt-3">
        ...
        <div className="mt-4 flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            <div className="space-y-4">
              ...
              <Card className="rounded-2xl">
                <CardContent className="p-4 text-xs text-muted-foreground flex items-center justify-between flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="h-4 w-4" />
                    <span>提示：所有按钮均可点击并有反馈（Toast/弹窗/状态变化）。</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="rounded-xl">版本：demo-mock-v2</Badge>
                    <Badge variant="secondary" className="rounded-xl">模式：{quickMode ? "快" : "准"}</Badge>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="rounded-2xl">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="text-sm font-semibold">一键走完整演示（Mock）</div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <Button
                        className="rounded-2xl"
                        onClick={() => {
                          notify("导演模式：开始", "将按顺序跳转并提示你点什么（Mock）");
                          setActive("health");
                        }}
                      >
                        从体检开始
                      ...
                      <Button
                        ...
                        className="rounded-2xl"
                        onClick={() => {
                          notify("导演模式：完成", "建议演示顺序：体检→清洗→任务→结果→解释→交付→对比（Mock）");
                          openModal(
                            "演示顺序（Mock）",
                            <div className="space-y-3">
                              <div className="text-sm">把“按钮能点”变成“观众能跟得上”的节奏。</div>
                              <div className="grid gap-2">
                                {["数据体检：一键体检+口径映射", "清洗方案：生成策略→预览→应用 v2", "任务：跑分类/回归/填补", "结果：分组误差+阈值收益→发布策略", "解释：全局/分群/单案/反事实", "交付：报告+API+告警", "对比：同口径证明墙"].map((x) => (
                                  <div key={x} className="p-3 rounded-2xl border text-sm">{x}</div>
                                ))}
                              </div>
                              <Button className="rounded-2xl" onClick={() => notify("已复制", "演示脚本已复制（Mock）")}>复制脚本</Button>
                            </div>
                          );
                        }}
                      >
