...
| 文档版本 | 修改日期 | 修改人 | 修改内容 |
| :--- | :--- | :--- | :--- |
| V5.7 | 2026-01-04 | 智能助手 | UI 优化：移除顶部导航栏中的“体检分”展示 |
| V5.6 | 2026-01-04 | 智能助手 | 功能增强：在 8.6 因果解释模块增加“样本行选择”功能，支持自定义行范围过滤分析；UI 优化：修正数据清洗规则 ID 显示顺序，移除平台标题“(演示端)”字样 |
...
| **样本行选择** | 支持用户选择数据行的范围（从第2行到最后一行），聚焦分析特定样本集。 | 输入框/按钮 | `Range`: 起始行-结束行 |
...
| **配置区** | 样本行范围 | Input Number | `Start/End Row` | 设置分析的样本行起始和结束位置 (2-31)。 |
